...
const int shmem_size_block_gemm = (((SUB_M_BLOCK / NUM_PIPE_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK) * NUM_RANK_BLOCK) + ((SUB_N_BLOCK / NUM_PIPE_N_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK) * NUM_RANK_BLOCK)) * sizeof(double);
...
__global__ void block_gemm_2d_double_mma(const double *A, const double *B, double *C, const double alpha, const double beta)
{
    extern __shared__ __align__(16) double shmem[];

    double *shmem_a = shmem;
    double *shmem_b = shmem + (SUB_M_BLOCK / NUM_PIPE_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK) * NUM_RANK_BLOCK;

    const unsigned int warp_id = threadIdx.x >> 5;
    const unsigned int lane_id = threadIdx.x & 31;

    const unsigned int warp_id_row = warp_id / NUM_RANK_BLOCK;
    const unsigned int warp_id_col = warp_id % NUM_RANK_BLOCK;
    ...
    double (*a_compute)[SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK][4 * WARP_SIZE] = reinterpret_cast<double (*)[SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK][4 * WARP_SIZE]>(shmem_a + warp_id_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK)));
    double (*b_compute)[SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK][2 * WARP_SIZE] = reinterpret_cast<double (*)[SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK][2 * WARP_SIZE]>(shmem_b + warp_id_col * ((SUB_K_BLOCK / NUM_PIPE_K_BLOCK) * (SUB_N_BLOCK)));
    ...
    for (int warp_row = 0; warp_row < SUB_M_BLOCK / WMMA_M_BLOCK; warp_row++)
    {
        for (int warp_col = 0; warp_col < SUB_K_BLOCK / WMMA_K_BLOCK; warp_col++)
        {
            ...
            int warp_col_offset = warp_id_col * SUB_K_BLOCK + warp_col * WMMA_K_BLOCK;
    ...
    for (int warp_row = 0; warp_row < SUB_N_BLOCK / WMMA_N_BLOCK; warp_row++)
    {
        for (int warp_col = 0; warp_col < SUB_K_BLOCK / WMMA_K_BLOCK; warp_col++)
        {
            ...
            int warp_col_offset = warp_id_row * SUB_K_BLOCK + warp_col * WMMA_K_BLOCK;
                        ...
                        if (warp_id_col == idx_stage)
                        {
                            for (int i = 0; i < SUB_M_BLOCK / NUM_PIPE_M_BLOCK / WMMA_M_BLOCK; i++)
                            {
                                for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK; j++)
                                {
                                    ...
                                    a_compute[i][j][0 + lane_id * 4] = a_frags[real_i][real_j][0];
                                    a_compute[i][j][1 + lane_id * 4] = a_frags[real_i][real_j][1];
                                    a_compute[i][j][2 + lane_id * 4] = a_frags[real_i][real_j][2];
                                    a_compute[i][j][3 + lane_id * 4] = a_frags[real_i][real_j][3];
                                }
                            }
                        }
                        ...
                        if (warp_id_row == idx_stage)
                        {
                            for (int i = 0; i < (SUB_N_BLOCK / NUM_PIPE_N_BLOCK) / WMMA_N_BLOCK; ++i)
                            {
                                for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK; ++j)
                                {
                                    ...
                                    b_compute[i][j][0 + lane_id * 2] = b_frags[real_i][real_j][0];
                                    b_compute[i][j][1 + lane_id * 2] = b_frags[real_i][real_j][1];
                                }
                            }
                        }
...
#pragma unroll
                                for (int k = 0; k < SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK; k++)
                                {
                                    ...
                                    mma_m16n8k8(c_frags[real_i][real_j], &a_compute[i][k][lane_id * 4], &b_compute[j][k][lane_id * 2]);
                                }
                            }
                        }
                    }
...
int main(int argc, char *argv[])
{
    ...
    cudaFuncSetAttribute(block_gemm_2d_double_mma, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_size_block_gemm);
    ...
    block_gemm_2d_double_mma<<<BLOCKS_PER_GRID, NUM_ALLOC_RANK_BLOCK * WARP_SIZE, shmem_size_block_gemm>>>(d_A, d_B, d_C, 1.0, 0.0);
    ...
    std::cout << "[hemeng_log],2d," << M_BLOCK << "," << N_BLOCK << "," << K_BLOCK << "," << NUM_RANK_BLOCK << "," << NUM_ALLOC_RANK_BLOCK << "," << tflops << "," << NUM_ALLOC_RANK_BLOCK * 32 << std::endl;
