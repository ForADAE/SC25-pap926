...
__global__ void block_gemm_3d_double_mma(const double *A, const double *B, double *C, const double alpha, const double beta)
{
    ...
    const unsigned int dep_offset_a = warp_id_dep * (SUB_M_BLOCK / NUM_PIPE_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK);
    const unsigned int row_offset_a = warp_id_row * (SUB_M_BLOCK / NUM_PIPE_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK) * NUM_RANK_BLOCK;

    const unsigned int dep_offset_b = warp_id_dep * (SUB_N_BLOCK / NUM_PIPE_N_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK);
...
#pragma unroll
                                for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK; ++j)
                                {
                                    ...
                                    a_compute[i][j][lane_id * 4 + 0] = a_frags[real_i][real_j][0];
                                    a_compute[i][j][lane_id * 4 + 1] = a_frags[real_i][real_j][1];
                                    a_compute[i][j][lane_id * 4 + 2] = a_frags[real_i][real_j][2];
                                    a_compute[i][j][lane_id * 4 + 3] = a_frags[real_i][real_j][3];
                                }
                            }
                        }
...
int main(int argc, char *argv[])
{
    ...
    cudaMalloc(&d_C, M_BLOCK * N_BLOCK * NUM_RANK_BLOCK * sizeof(double));
    ...
    cudaMemset(d_C, 0, M_BLOCK * N_BLOCK * NUM_RANK_BLOCK * sizeof(double));

    cudaEvent_t start, stop;
    cudaEventCreate(&start);
    cudaEventCreate(&stop);

    cudaFuncSetAttribute(block_gemm_3d_double_mma, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_size_block_gemm);
    ...
    block_gemm_3d_double_mma<<<BLOCKS_PER_GRID, NUM_ALLOC_RANK_BLOCK * WARP_SIZE, shmem_size_block_gemm>>>(d_A, d_B, d_C, 1.0, 0.0);
    ...
    std::cout << "[hemeng_log],3d," << M_BLOCK << "," << N_BLOCK << "," << K_BLOCK << "," << NUM_RANK_BLOCK << "," << NUM_ALLOC_RANK_BLOCK << "," << tflops << "," << NUM_ALLOC_RANK_BLOCK * 32 << std::endl;

    double *h_C = (double *)malloc(M_BLOCK * N_BLOCK * NUM_RANK_BLOCK * sizeof(double));
    cudaMemcpy(h_C, d_C, M_BLOCK * N_BLOCK * NUM_RANK_BLOCK * sizeof(double), cudaMemcpyDeviceToHost);
    ...
    for (int i = 0; i < M_BLOCK; i++)
    {
        for (int j = 0; j < N_BLOCK; j++)
        {
            ...
            {
                h_C[i * N_BLOCK + j] += h_C[i * N_BLOCK + j + k * M_BLOCK * N_BLOCK];
            }
        }
    }
